...
class Blockchain {
  ...
  getDifficulty (index, refBlockchain = this.blocks, mixChains = false) {
    ...
    if (mixChains) {
        mixedChain.splice(0, this.blocks.length, ...this.blocks);
        //mixedChain = [].concat(this.blocks, mixedChain);
    }
    return Config.pow.getDifficulty(/*(mixChains && index >= this.blocks.length) ? mixedChain : this.blocks*/ mixedChain, index)
  }
  ...
  replaceChain (newBlockchain, reorgToBlock = null) {
    ...
    this.checkChain(newBlockchain, (reorgToBlock !== null))
    ...
    R.forEach((block) => {
      this.addBlock(block, false, (reorgToBlock !== null))
    }, newBlocks)
  ...
  checkBlock (newBlock, previousBlock, referenceBlockchain = this.blocks, reorg = false) {
    ...
    if (previousBlock.index + 1 !== newBlock.index) { // Check if the block is the last one
      ...
      if (newBlock.getDifficulty() >= this.getDifficulty(newBlock.index, referenceBlockchain, !reorg)) { // If the difficulty level of the proof-of-work challenge is correct
        console.error(`Invalid proof-of-work difficulty: expected '${newBlock.getDifficulty()}' to be smaller than '${this.getDifficulty(newBlock.index, referenceBlockchain, true)}' on Block index ${newBlock.index}`)
