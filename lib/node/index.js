...
class Node {
  constructor (host, port, peers, blockchain) {
    ...
    this.hookBlockchain()
    if(this.host !== '::' && this.host !== 'localhost' && this.host !== '127.0.0.1') //Make sure that we aren't broadcasting localhost to other peers
      this.connectToPeers(peers)
    else if(peers.length > 0) //Try to retrieve a public address from other peer
      superagent
          .get(`${peers[0].url}/ip`)
          .then((res)=>{
            this.host = res.body.ip;
            this.connectToPeers(peers);
          }).catch((err)=>{console.warn("Could not fetch IP from other node!!!!", err);})
  }
  ...
  getLatestBlock (peer) {
    ...
    return superagent
      ...
      .then((res) => {
        ...
        self.checkReceivedBlock(Block.fromJson(res.body), peer)
      })
      .catch((err) => {
  ...
  getBlocks (peer) {
    ...
    return superagent
      ...
      .catch((err) => {
        console.warn(`Unable to get blocks from ${URL}: ${err.stack}`)
        this.setSyncing(false)
      })
  }
  ...
  checkReceivedBlocks (blocks, peer) {
    ...
    if (latestBlockHeld.hash === latestBlockReceived.previousHash) { // We can append the received block to our chain
      ...
      if (forkTip !== null) {
        ...
        console.info(`Valid fork found at index ${forkTip.index}, preparing local blockchain reorganization...`)
